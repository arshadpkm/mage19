 ...
 * Time: 2:57 PM
 */
class Retailon_Marketplace_Model_Singleorder extends Mage_Core_Model_Abstract
{
    ...
    {
        $this->_init('marketplace/singleorder');
    }
    ...
    public function createOrder($products)
    {
        ...
        try{
            $reservedOrderId = $this->_orderId;

//        Mage::log($reservedOrderId,null,'singleorder.log');
//
...
//        Mage::log($this->_shippingMethod,null,'singleorder.log');

            $this->_order->setShippingAddress($shippingAddress)
                ->setShippingMethod($this->_shippingMethod);

//        Mage::log($this->_paymentMethod,null,'singleorder.log');
            ...
            $this->_order->setIsSingleOrder(1);
    ...
    public function saveVendor($product_id)
    {
        ...
        if ( $vendor->getSize() > 0 ) {
            ...
            $newVendor = Mage::getModel('marketplace/vendor');
            $vendor_id = $vendor->getUserId();
            $newVendor->setData('mage_order_id',$this->_orderId);
            $newVendor->setData('user_id',$vendor_id);

//            Mage::log('$vendor_id'.$vendor_id,null,'newvendor.log');
//            Mage::log('$this->_orderId'.$this->_orderId,null,'newvendor.log');
